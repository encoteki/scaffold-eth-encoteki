...
contract DAOImplementation is Initializable, OwnableUpgradeable {
  ...
  // Events
  // --------------------------------------------------
  event InitializeProposal(address indexed proposalAddress, address owner, string proposalName, address erc721Address);

  event ProposalVoted(uint256 indexed tokenId, address indexed voter, uint256 indexed optionId);
  ...
  function initialize(
    ...
    emit InitializeProposal(address(this), owner(), proposalName, erc721Address);
  }
  ...
  function voteDAO(uint256 tokenId, uint256 optionId) external {
    ...
    if (IERC721(erc721Address).ownerOf(tokenId) != msg.sender) revert NotTokenOwner(tokenId);
  ...
  function canVote(address voter, uint256 tokenId) external view returns (bool) {
